...
public class NguoiDungPage extends BasePage {
        ...
        try {
            ...
            for (NguoiDung user : users) {
                ...
                if (applyFilter && !searchQuery.isEmpty()) {
                    ...
                    if (!idStr.contains(searchQuery) && !username.contains(searchQuery) && !fullName.contains(searchQuery)) {
                        continue;
                    }
                }
            ...
            JOptionPane.showMessageDialog(this, "Lỗi tải dữ liệu người dùng: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        }
    }
    ...
    protected void handleEdit() {
        ...
        if (selectedId == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn một người dùng từ bảng để chỉnh sửa.", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        try {
            ...
            if (optionalUser.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Không tìm thấy thông tin người dùng được chọn.", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            JOptionPane.showMessageDialog(this, "Lỗi khi lấy thông tin người dùng: " + ex.getMessage(), "Lỗi hệ thống", JOptionPane.ERROR_MESSAGE);
            ex.printStackTrace();
        }
    }
    ...
    protected void handleDelete() {
        int selectedId = getSelectedId();
        if (selectedId == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn một người dùng từ bảng để xóa.", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }

        if (AppSys.getNguoiDung() != null && AppSys.getNguoiDung().getUserId().equals(selectedId)) {
            JOptionPane.showMessageDialog(this, "Bạn không thể xóa tài khoản của chính mình đang đăng nhập!", "Lỗi bảo mật", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                ...
                JOptionPane.showMessageDialog(this, "Lỗi khi xóa người dùng: " + ex.getMessage(), "Lỗi hệ thống", JOptionPane.ERROR_MESSAGE);
                ex.printStackTrace();
            }
        }
    }
